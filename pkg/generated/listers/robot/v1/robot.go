...
type RobotLister interface {
	...
	List(selector labels.Selector) (ret []*v1.Robot, err error)
...
// List lists all Robots in the indexer.
func (s *robotLister) List(selector labels.Selector) (ret []*v1.Robot, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Robot))
	})
	return ret, err
}
...
type RobotNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.Robot, error)
...
// List lists all Robots in the indexer for a given namespace.
func (s robotNamespaceLister) List(selector labels.Selector) (ret []*v1.Robot, err error) {
...
// Get retrieves the Robot from the indexer for a given namespace and name.
func (s robotNamespaceLister) Get(name string) (*v1.Robot, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("robot"), name)
	}
	return obj.(*v1.Robot), nil
}
